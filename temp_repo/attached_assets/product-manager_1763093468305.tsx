  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      for (const colorId of data.selectedColors) {
        await apiRequest(`/api/models/${model.id}/colors/${colorId}`, {
          method: "POST",
        });
      }
      ...
      for (const storageId of data.selectedStorage) {
        await apiRequest(`/api/models/${model.id}/storage/${storageId}`, {
          method: "POST",
        });
      }
          ...
          {models.map((model) => {
            ...
            return (
              <TableRow key={model.id}>
                ...
                <TableCell>{model.generation || "—"}</TableCell>
...
function StorageManager({ 
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return await apiRequest("/api/storage-options", {
        method: "POST",
        body: JSON.stringify({
          ...data,
    ...
    onSuccess: () => {
      ...
      setIsDialogOpen(false);
      setFormData({ name: "", nameFa: "", categoryId: "", order: 0 });
...
function ProductPriceManager({
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return await apiRequest("/api/product-prices", {
        method: "POST",
        body: JSON.stringify(data),
        headers: { "Content-Type": "application/json" },
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { price: number } }) => {
      return await apiRequest(`/api/product-prices/${id}`, {
        method: "PATCH",
        body: JSON.stringify(data),
        headers: { "Content-Type": "application/json" },
      });
